...
def sulfide_inh_psb(resp, S, K_s, K_i):
    """
    Forcing function for hydrogen sulfide inhibition in purple sulfur bacteria.
    ...
    resp : NoneType
        Placeholder for NutMEG forcing function interface
    S : float
        Concentration of hydrogen sulfide (µM)
    K_s : float
        Half-saturation constant for H2S uptake (µM)
    K_i : float
...
def get_psb(R, rxn, Pm, I, k_l_psb, NO3, NH4, P, O2, H2S, R_no3, R_a, k_p_psb, k_h2s_psb, a_inh, O2_inh, K_s, K_i, mmr, mgr, num=1e6, name='Purple Sulfur Bacterium'):
    """
    ...
    _phototroph = nm.horde(
        ...
        CHNOPS_kwargs={
            ...
            'CHNOPS_forcing_parameters': {
                ...
                # 'Oxygen': (inhibition, ['O2', 'a_inh', 'O2_inh']),
                'Sulfur': (sulfide_inh_psb, ['H2S', 'K_s', 'K_i'])
            }, 
            'CHNOPS_F_attrs': {
                'Pmax': Pm,
                'I': I, 'k_l_psb': k_l_psb,
                ...
                'a_inh': a_inh, 'O2_inh': O2_inh, 'K_s': K_s, 'K_i': K_i
            }
        }
    )
...
def main():
    ...
    mu_max_psb = 0.25 / 86400  # Maximum growth rate for green sulfur bacteria (/s)
    mu_max_psb = 0.05 / 3600 # psb mu max
    ...
    H2S_inh_opnnf = 1.0  # Inhibition constant for H2S in OPNNF (μM)
    k_l_opnnf = 100.0 # Light limitation constant for non-nitrogen fixing oxygenic phototrophs (μmol photons m⁻²) <- check units

    k_p_psb = 1.6 # Phosphorus limitation for green sulfur bacteria (μM)
    k_h2s_psb = 2.0 # Sulfide limitation for green sulfur bacteria (μM)
    O2_inh = 1.0 # Inhibition constant for O2 (μM)
    k_l_psb = 1.0 # Light limitation constant for green sulfur bacteria (μmol photons m⁻²) <- check units
    ...
    F_S_inh_opnnf = inhibition(None, np.nan_to_num(data['H2S'], nan=0.0), a_inh, H2S_inh_opnnf)

    # ---- PSB ----
    # Irradiance forcing factor
    F_I_psb = Monod(None, data['par'], k_l_psb)
    ...
    prod_psb = np.full(len(data['depth']), np.nan)
    ...
    for i in range(len(data['depth'])):
        ...
        if not (np.isnan(data['NO3'][i]) and np.isnan(data['NH4'][i])):
            ...
            # OPNNF
            try:
                opnnf = get_opnnf(R, rxn, Pm, data['par'][i], k_l_opnnf, alpha, NO3_calc, NH4_calc, data['P'][i], H2S_calc, R_no3, R_a, k_p_opnnf, a_inh, H2S_inh_opnnf, mmr, mu_max_opnnf * temperature_modifier(data['temp'][i], k_t))
            ...
            # PSB
            try:
                psb = get_psb(R, rxn, Pm, data['par'][i], k_l_psb, NO3_calc, NH4_calc, data['P'][i], data['O2'][i], H2S_calc, R_no3, R_a, k_p_psb, k_h2s_psb, a_inh, O2_inh, K_s, K_i, mmr, mu_max_psb * temperature_modifier(data['temp'][i], k_t))
                prod_psb[i] = get_phototroph_rate(psb)
            except:
                prod_psb[i] = np.nan
    ...
    # Row 1, Plot 3: PSB Forcing factors
    axes[0, 2].plot(F_I_psb, data['depth'], label='F_I (irradiance)', linewidth=1, color='orange')
    axes[0, 2].plot(F_N, data['depth'], label='β_t (total nitrogen availability)', linewidth=1, color='blue')
    axes[0, 2].plot(F_P_psb, data['depth'], label='F_P (total phosphorus availability)', linewidth=1, color='green')
    # axes[0, 2].plot(F_H2S_psb, data['depth'], label='F_H2S (total sulfur availability)', linewidth=1, color='purple')
    # axes[0, 2].plot(F_O2_inh, data['depth'], label='F_O2 (O2 inhibition)', linewidth=1, color='red')
    axes[0, 2].plot(F_H2S_inh_psb, data['depth'], label='F_H2S (H2S inhibition)', linewidth=1, color='purple')
    axes[0, 2].invert_yaxis()
    axes[0, 2].set_xlim(-0.03, 1.03)
    axes[0, 2].set_ylim(max_graphing_depth, 0)
    axes[0, 2].set_xlabel('Forcing Factor', fontsize=10)
    axes[0, 2].set_title('PSB Forcing Factors', fontsize=13, fontweight='bold')
